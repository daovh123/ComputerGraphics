...
export default function Layout({ 
  ...
  const lessons = [
    { 
      ...
      sections: [
        { id: "lesson-overview", label: "Khái quát cơ thể người", icon: BookOpen },
        { id: "body-parts", label: "Cấu tạo cơ thể", icon: Layers },
        { id: "coordination", label: "Sự phối hợp cơ thể", icon: Activity },
        { id: "biological-clock", label: "Đồng hồ sinh học", icon: Clock },
        { id: "summary", label: "Tổng kết kiến thức", icon: CheckSquare },
        { id: "challenge", label: "Giải trí & Thử thách", icon: Gamepad2 },
        { id: "characteristics", label: "Đặc điểm riêng", icon: User },
      ]
    },
    { id: "31", label: "Bài 31", fullName: "Hệ vận động ở người", sections: [] },
    { id: "32", label: "Bài 32", fullName: "Dinh dưỡng và tiêu hóa", sections: [] },
    { id: "33", label: "Bài 33", fullName: "Máu và hệ tuần hoàn", sections: [] },
    { id: "34", label: "Bài 34", fullName: "Hệ hô hấp ở người", sections: [] },
    { id: "35", label: "Bài 35", fullName: "Hệ bài tiết ở người", sections: [] },
  ];
  ...
  return (
    <div className="flex h-screen bg-[#F0F8FF] font-sans text-[#333]">
      ...
      >
        <div className={cn("py-3 px-6 flex items-center justify-between", isSidebarCollapsed && "px-4")}>
          <div 
        ...
        <nav className="flex-1 px-4 py-4 space-y-1 overflow-y-auto custom-scrollbar overflow-x-hidden">
          ...
          >
            <Home className={cn("w-5 h-5 min-w-[20px]", currentView === "dashboard" && selectedSubjectId === "" ? "text-[#00BFFF]" : "text-[#BBB]")} />
            {!isSidebarCollapsed && <span className="font-bold text-sm whitespace-nowrap">Trang chủ</span>}
          </button>
          ...
          >
            <Library className={cn("w-5 h-5 min-w-[20px]", selectedSubjectId !== "" ? "text-[#00BFFF]" : "text-[#BBB]")} />
            {!isSidebarCollapsed && <span className="font-bold text-sm whitespace-nowrap">Môn học</span>}
            {!isSidebarCollapsed && <ChevronDown className={cn("w-4 h-4 ml-auto transition-transform", !isSubjectsExpanded ? "-rotate-90" : "")} />}
          </button>
              ...
              >
                {SUBJECTS.map((subject) => (
                  <div key={subject.id} className="space-y-1">
                    ...
                    >
                      <subject.icon className={cn("w-4 h-4", selectedSubjectId === subject.id ? "text-[#00BFFF]" : "text-[#BBB]")} />
                      <span className="text-sm">{subject.name}</span>
                      {subject.id === "khtn" && (
                        <ChevronRight className={cn("w-3 h-3 ml-auto transition-transform", isKHTNExpanded ? "rotate-90" : "")} />
                      )}
                    </button>
                    ...
                    {subject.id === "khtn" && (
                      <AnimatePresence>
                        {isKHTNExpanded && (
                          ...
                          >
                            {lessons.map((lesson) => {
                              const isLessonActive = isSectionActive(lesson.id) || (currentView === "lesson-placeholder" && selectedLessonTitle === lesson.fullName);
                              return (
                                <div key={lesson.id} className="space-y-1">
                                  <button
                                    onClick={() => {
                                      if (lesson.sections.length > 0) {
                                        ...
                                        setCurrentView(lesson.sections[0].id as View);
                                      } else {
                                  ...
                                  >
                                    <FileText className={cn("w-4 h-4", isLessonActive ? "text-[#00BFFF]" : "text-[#BBB]")} />
                                    <span className="text-sm">{lesson.label}</span>
                                    {lesson.sections.length > 0 && (
                                      <ChevronRight className={cn("w-3 h-3 ml-auto transition-transform", expandedLesson === lesson.id ? "rotate-90" : "")} />
                                    )}
                                  </button>

                                  {/* Sections Level */}
                                  <AnimatePresence>
                                    {expandedLesson === lesson.id && lesson.sections.length > 0 && (
                                      <motion.div
                                        initial={{ height: 0, opacity: 0 }}
                                        animate={{ height: "auto", opacity: 1 }}
                                        exit={{ height: 0, opacity: 0 }}
                                      ...
                                      >
                                        {lesson.sections.map((section) => (
                                          <button
                                            key={section.id}
                                            onClick={() => setCurrentView(section.id as View)}
                                            className={cn(
                                          ...
                                          >
                                            <section.icon className={cn("w-3.5 h-3.5", currentView === section.id ? "text-[#00BFFF]" : "text-[#CCC] group-hover:text-[#00BFFF]")} />
        ...
        <div className={cn("p-6 border-t border-[#E0F0FF]", isSidebarCollapsed && "px-4")}>
          <button className={cn("w-full flex items-center gap-3 px-4 py-3 text-[#666] hover:text-[#00BFFF] transition-colors", isSidebarCollapsed && "px-0 justify-center")}>
            <Settings className="w-5 h-5 min-w-[20px]" />
            {!isSidebarCollapsed && <span className="font-medium text-sm">Cài đặt</span>}
          </button>
        </div>
      </motion.aside>
            ...
            <nav className="flex items-center gap-6">
              <button 
                ...
                className={cn(
                  ...
                  currentView === "dashboard" || currentView.startsWith("lesson-") 
              ...
              <button className="text-[#666] font-medium hover:text-[#00BFFF] transition-colors">Tài nguyên</button>
            </nav>
